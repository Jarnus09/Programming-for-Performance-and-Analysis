...
int avx2_version(const int *source, int *dest)
{
  ...
  for (int i = 0; i < N; i += stride)
  {
    ...
    __m256i x = _mm256_load_si256((__m256i const *)&source[i]);

    __m256i tmp0 = _mm256_slli_si256(x, 4);
    __m256i tmp1 = _mm256_add_epi32(x, tmp0);

    __m256i tmp2 = _mm256_slli_si256(tmp1, 8);
    __m256i tmp3 = _mm256_add_epi32(tmp1, tmp2);

    __m128i lo128 = _mm256_castsi256_si128(tmp3);
    __m128i lo_sum = _mm_shuffle_epi32(lo128, _MM_SHUFFLE(3, 3, 3, 3));

    __m256i lo_broadcast = _mm256_castsi128_si256(_mm_setzero_si128());
    __m256i hi_broadcast = _mm256_inserti128_si256(lo_broadcast, lo_sum, 1);

    __m256i out = _mm256_add_epi32(tmp3, hi_broadcast);

    out = _mm256_add_epi32(out, offset);

    _mm256_store_si256((__m256i *)&dest[i], out);

    __m128i hi128 = _mm256_extracti128_si256(out, 1);
    __m128i last = _mm_shuffle_epi32(hi128, _MM_SHUFFLE(3, 3, 3, 3));
    offset = _mm256_broadcastd_epi32(last);
  }
  return dest[N - 1];
